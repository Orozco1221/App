            ...
            <div className="p-10 rounded-[4.4rem] bg-white/70 backdrop-blur-3xl relative overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between mb-12 gap-6 relative z-10 text-[#1e2b7a]">
                <div className="flex items-center gap-6">
                  <div className={`p-5 rounded-[2rem] shadow-xl ${cat === "cafeteria" ? "bg-amber-100 text-amber-600" : cat === "pills" ? "bg-purple-100 text-purple-600" : cat === "structural" ? "bg-blue-100 text-blue-600" : "bg-green-100 text-green-600"}`}>
                  ...
                  <div>
                    ...
                    <p className="text-xs font-black uppercase mt-2 tracking-[0.2em] opacity-40 italic leading-none">
                      {cat === "cafeteria" ? "Inspiracion de 30 min para expertos" : cat === "pills" ? "Videos rapidos con impacto real" : cat === "structural" ? "Conocimiento Core Randstad" : "Formacion recomendada de mercado"}
                    </p>
                  </div>
                </div>

                <button
                  onClick={() => { setTargetCategory(cat); setShowAddModal(true); }}
                  aria-label={`Anadir material a ${cat === "cafeteria" ? "CafeterIA" : cat === "pills" ? "TikTok Learning" : cat === "structural" ? "Estructurales" : "Certificaciones"}`}
                  className={`px-6 py-4 text-white rounded-[2rem] shadow-2xl hover:scale-105 transition-all flex items-center gap-2 font-black uppercase text-[10px] italic tracking-widest leading-none group ${cat === "cafeteria" ? "bg-amber-600" : cat === "pills" ? "bg-purple-600" : cat === "structural" ? "bg-[#3b82f6]" : "bg-green-600"}`}
                >
                  <Plus size={20} className="group-hover:rotate-90 transition-transform" /> Anadir Material
                </button>
              </div>
                    ...
                    {cat === "pills" ? (
                      ...
                      <div className="h-full flex flex-col text-[#1e2b7a]">
                        ...
                        <div className="mt-auto border-t border-slate-50 pt-8 flex justify-between items-center text-[10px] font-black text-slate-300 uppercase tracking-widest leading-none">
                          <span>{item.provider || "RANDSTAD"}</span>
