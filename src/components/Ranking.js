...
// Recibimos los datos del ranking ordenado, el podio, el usuario actual y el estado del Mentor IA
const Ranking = ({
  sortedRanking,
  myIndex,
  podiumUsers,
    ...
    <div className="animate-in fade-in duration-500 space-y-16 text-[#1e2b7a]">
      ...
      <ScrollReveal direction="none">
        <div className="text-center space-y-4 mb-16">
          <h2 className="text-5xl font-black uppercase italic tracking-tighter text-[#1e2b7a] leading-none">LÍDERES DE ADOPCIÓN IA</h2>
          <p className="text-[#94a3b8] font-bold text-xs uppercase tracking-[0.3em] leading-none">RECONOCIMIENTO AL TALENTO Y LA INNOVACIÓN</p>
        </div>
      </ScrollReveal>
          ...
          <div className="order-2 md:order-1 flex-1 max-w-[280px] w-full">
            <ScrollReveal direction="up" delay={200}>
              <div className="flex flex-col items-center">
                <div className="w-20 h-20 rounded-3xl bg-white shadow-xl flex items-center justify-center border-4 border-slate-200 mb-4 relative uppercase leading-none">
                  <span className="text-2xl font-black text-slate-300 italic">{podiumUsers[1].avatar}</span>
                  <div className="absolute -top-3 -right-3 bg-slate-300 text-white p-1.5 rounded-xl shadow-lg"><Medal size={20}/></div>
                </div>
                <div className="bg-white w-full p-8 rounded-t-[3rem] shadow-sm border-x border-t border-slate-100 h-40 text-center flex flex-col justify-center">
                  <p className="text-[10px] font-black text-slate-400 mb-1 leading-none">#2</p>
                  <p className="font-black uppercase italic text-sm leading-tight leading-none">{podiumUsers[1].name}</p>
        ...
        {podiumUsers[0] && (
          <div className="order-1 md:order-2 flex-1 max-w-[320px] w-full z-10">
            <ScrollReveal direction="up" delay={0}>
              <div className="flex flex-col items-center">
                <div className="w-28 h-28 rounded-[2.5rem] bg-white shadow-2xl flex items-center justify-center border-4 border-[#f59e0b] mb-4 relative scale-110 uppercase leading-none">
                ...
                <div className="bg-white w-full p-10 rounded-t-[4rem] shadow-2xl border-x border-t border-[#f1f5f9] h-56 text-center flex flex-col justify-center ring-4 ring-[#3b82f6]/5">
                  <p className="text-[11px] font-black text-[#f59e0b] mb-1 leading-none">#1</p>
                  <p className="font-black uppercase italic text-lg leading-tight leading-none">{podiumUsers[0].name}</p>
                  ...
                  <span className="text-[9px] font-black bg-blue-50 text-[#3b82f6] px-3 py-1 rounded-full w-fit mx-auto mt-4 tracking-widest leading-none">
                    {podiumUsers[0].tier}
                  </span>
                </div>
              </div>
        ...
        {podiumUsers[2] && (
          <div className="order-3 md:order-3 flex-1 max-w-[280px] w-full">
            <ScrollReveal direction="up" delay={400}>
              <div className="flex flex-col items-center">
                <div className="w-20 h-20 rounded-3xl bg-white shadow-xl flex items-center justify-center border-4 border-amber-600/30 mb-4 relative uppercase leading-none">
                  <span className="text-2xl font-black text-amber-700/40 italic">{podiumUsers[2].avatar}</span>
                  <div className="absolute -top-3 -right-3 bg-amber-700 text-white p-1.5 rounded-xl shadow-lg"><Medal size={20}/></div>
                </div>
                <div className="bg-white w-full p-8 rounded-t-[3rem] shadow-sm border-x border-t border-slate-100 h-32 text-center flex flex-col justify-center">
                  <p className="text-[10px] font-black text-slate-400 mb-1 leading-none">#3</p>
                  <p className="font-black uppercase italic text-sm leading-tight leading-none">{podiumUsers[2].name}</p>
      ...
      {/* Banner de Posición Actual del Usuario y AI Mentor */}
      <ScrollReveal direction="none">
        <div className="max-w-4xl mx-auto bg-gradient-to-r from-[#1e2b7a] to-[#3b82f6] p-8 rounded-[3rem] shadow-2xl text-white flex flex-col gap-6 relative overflow-hidden group mb-12">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div className="flex items-center gap-8">
              <div className="w-20 h-20 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 flex items-center justify-center font-black text-2xl text-white shadow-inner uppercase leading-none">
                {CURRENT_USER_ID}
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-200 mb-1 leading-none">TU POSICIÓN ACTUAL</p>
                <h4 className="text-2xl font-black uppercase italic tracking-tighter leading-none">
                  JUAN PÉREZ <span className="text-blue-300 opacity-50 ml-2">#{sortedRanking[myIndex]?.position}</span>
                </h4>
              </div>
            </div>
            ...
            <div className="text-right">
              ...
              <span className="text-[9px] font-black bg-white text-[#1e2b7a] px-3 py-1 rounded-lg uppercase tracking-widest mt-3 inline-block leading-none">
                {sortedRanking[myIndex]?.tier}
              </span>
            </div>
          </div>
          
          {/* Botón Mágico del Mentor IA */}
          <div className="relative z-10 pt-4 border-t border-white/10 flex flex-col md:flex-row gap-4">
            ...
            >
              <Lightbulb size={14} /> ✨ Mentor de IA ✨
            </button>
            {mentorResponse && (
              <div className="flex-1 p-4 bg-white/10 rounded-2xl text-[11px] font-medium italic animate-in fade-in slide-in-from-top-2 leading-relaxed">
                "{mentorResponse}"
              </div>
            )}
          </div>
          ...
          {/* Decoración gráfica */}
          <Sparkles className="absolute -right-10 top-0 text-white/5 w-64 h-64 -rotate-12 pointer-events-none" />
        </div>
      </ScrollReveal>

      {/* Lista del Top Completo (Tabla) */}
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between px-10 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 leading-none text-right">
          ...
          <span className="text-right uppercase">TIER / TOTAL XP</span>
        </div>
